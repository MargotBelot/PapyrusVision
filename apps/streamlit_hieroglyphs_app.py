...
def resize_image_for_model(image, min_size=800, max_size=1333):
    """
    Resize image to match training data dimensions used by the Detectron2
    model.
    """
        ...
        if os.path.exists(unicode_file):
            ...
            for gardiner_code, info in unicode_data.items():
                unicode_mapping[gardiner_code] = {
                    'unicode_codes': info.get(
                        'unicode_codes', []), 'description': info.get(
                        'description', 'Unknown'), 'unicode_symbol': get_unicode_symbol(
                        info.get(
                            'unicode_codes', []))}
...
def predict_hieroglyphs(
    ...
    if len(instances) > 0:
        ...
        for i in range(len(instances)):
            ...
            gardiner_code = model_info['category_names'][class_idx] if class_idx < len(
                model_info['category_names']) else f'Unknown_{class_idx}'
...
def create_visualization(image, results):
    ...
    if results['detections']:
        legend_elements = [
            plt.Rectangle(
                ...
                facecolor='lime',
                alpha=0.7,
                label=f"High confidence (≥0.8): {results['summary']['high_confidence_count']}"),
            plt.Rectangle(
                (0,
                 0),
                1,
                ...
                facecolor='cyan',
                alpha=0.7,
                label=f"Medium confidence (0.6-0.8): {results['summary']['medium_confidence_count']}"),
            plt.Rectangle(
                (0,
                 0),
                1,
                ...
                facecolor='orange',
                alpha=0.7,
                label=f"Low confidence (<0.6): {results['summary']['low_confidence_count']}")]
        ax.legend(
    ...
    # Set title
    title = "Hieroglyph Detection Results\n"
...
def show_detection_page():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">PapyrusVision Hieroglyph Detection</h1>',
        unsafe_allow_html=True)
    st.markdown(
        '<p class="sub-header">Interactive AI-powered analysis of ancient Egyptian hieroglyphs</p>',
        unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with st.expander("Model Information"):
            st.write(
                f"**Model**: {os.path.basename(sorted(glob.glob(os.path.join(MODELS_DIR, 'hieroglyph_model_*')))[-1])}")
            st.write(f"**Classes**: {len(model_info['category_names'])}")
            st.write(
                f"**Default threshold**: {model_info['detection_threshold']}")
        ...
        if not uploaded_file:
            ...
            if use_default:
                default_image_path = os.path.join(
                    DATA_DIR, 'images', '145_upscaled_bright.jpg')
    ...
    with col2:
        ...
        if uploaded_file:
            ...
            if (new_width, new_height) != (original_width, original_height):
                st.info(
                    f"Resized for model consistency: {new_width}x{new_height}")

            # Run prediction on resized image
            with st.spinner("Analyzing hieroglyphs..."):
                results = predict_hieroglyphs(
                    predictor,
                    model_info,
                    unicode_mapping,
                    resized_image,
            ...
            if results['summary']['total_detections'] > 0:
                st.success(
                    f"Found {results['summary']['total_detections']} hieroglyphs!")
                ...
                with met_col3:
                    st.metric(
                        "Max Confidence",
                        f"{results['summary']['confidence_stats']['max']:.3f}")
                with met_col4:
                    st.metric(
                        "Mean Confidence",
                        f"{results['summary']['confidence_stats']['mean']:.3f}")

            else:
                st.warning(
                    "No hieroglyphs detected. Try lowering the confidence threshold.")

    # Visualization section
    if uploaded_file and 'results' in locals(
    ) and results['summary']['total_detections'] > 0:
        ...
        for detection in results['detections']:
            ...
            valid_unicode_codes = [
                code for code in detection['unicode_codes'] if code.startswith('U+')]
...
def show_paleography_page():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">Digital Hieroglyph Paleography</h1>',
        unsafe_allow_html=True)
    st.markdown(
        '<p class="sub-header">Create comprehensive catalogs of hieroglyphic signs with cropping and Unicode mapping</p>',
        unsafe_allow_html=True)
    ...
    if not uploaded_files:
        use_default_paleo = st.checkbox(
            "Use default test image for paleography demonstration", value=True)
        if use_default_paleo:
            ...
            if os.path.exists(default_image_path):
                ...
                st.info("Using default test image: 145_upscaled_bright.jpg")
                st.info(
                    "This will demonstrate the paleography tool with a sample hieroglyphic text.")

    if uploaded_files:
        if hasattr(
                uploaded_files[0],
                'name') and uploaded_files[0].name == '145_upscaled_bright.jpg':
            st.success("Using default test image for demonstration")
        else:
            ...
            with st.spinner("Initializing Digital Paleography Tool..."):
                try:
                    ...
                    if not paleography_tool.analyzer:
                        st.error(
                            "Could not load the hieroglyph analyzer. Please check your model setup.")
                        st.stop()
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for uploaded_file in uploaded_files:
                    ...
                    if (new_width, new_height) != (
                        ...
                        st.info(
                            f"{uploaded_file.name}: Resized from {original_width}x{original_height} to {new_width}x{new_height} for consistent processing")
                ...
                for i, file_path in enumerate(file_paths):
                    ...
                    try:
                        crops_data = paleography_tool.process_image(
                ...
                if all_crops_data:
                    st.success(
                        f"Generated {len(all_crops_data)} sign crops from {len(file_paths)} images")

                    # Create HTML catalog
                    with st.spinner("Creating interactive HTML catalog..."):
                        catalog_file = paleography_tool.create_html_catalog(
                            all_crops_data)

                    # Generate report
                    with st.spinner("Generating detailed report..."):
                        paleography_tool.generate_report(all_crops_data)
                    ...
                    with stat_col2:
                        unique_codes = len(
                            set(crop['gardiner_code'] for crop in all_crops_data))
                        st.metric("Unique Gardiner Codes", unique_codes)

                    with stat_col3:
                        avg_confidence = np.mean(
                            [crop['confidence'] for crop in all_crops_data])
                        st.metric(
                            "Average Confidence",
                            f"{avg_confidence:.3f}")
                    ...
                    with stat_col4:
                        source_images = len(
                            set(crop['source_image'] for crop in all_crops_data))
                    ...
                    for i, crop in enumerate(all_crops_data[:5]):
                        with preview_cols[i]:
                            try:
                                ...
                                st.image(
                                    crop_image, caption=f"{crop['gardiner_code']} ({crop['confidence']:.2f})")
                            except Exception as e:
                    ...
                    with download_col2:
                        ...
                        with st.spinner("Creating ZIP package..."):
                            zip_file = create_paleography_zip(
                                all_crops_data, catalog_file)
                    ...
                    st.warning(
                        "No sign crops were generated. Try lowering the confidence threshold or adjusting the minimum size.")
...
def show_about_page():
    ...
    st.markdown(
        '<h1 class="main-header">About PapyrusVision</h1>',
        unsafe_allow_html=True)
